...
import tf_util
from utils import explained_variance, add_noise
...
def dict_to_array(dic):
    return [[k, str(v)] for k,v in sorted(dic.items())]
...
class PpoAgent(object):
    ...
    def collect_random_statistics(self, num_timesteps):
        ...
        num_timesteps = 0
        print("collect random statistics")

        all_ob = []
        for lump in range(self.I.nlump):
            all_ob.append(self.I.venvs[lump].reset())
        for step in tqdm(range(num_timesteps), ncols=70):
            for lump in range(self.I.nlump):
    ...
    def test(self, sli):
        ...
        # Calcuate the intrinsic rewards for test set
        fd = {}
        fd.update({self.stochpol.ph_mean: self.stochpol.ob_rms.mean,
                    self.stochpol.ph_std: self.stochpol.ob_rms.var ** 0.5})
        fd[self.stochpol.ph_ac] = self.I.buf_acs[sli]

        fd[self.stochpol.ph_ob[None]] = self.test_set[sli]
        distracted_test_rews_int = tf.get_default_session().run(self.stochpol.int_rew, fd)

        fd[self.stochpol.ph_ob[None]] = self.test_set_clean[sli]
        clean_test_rews_int = tf.get_default_session().run(self.stochpol.int_rew, fd)

        # contrast_score = distracted_test_rews_int / clean_test_rews_int
        # test_score = np.mean(contrast_score)
        test_score = np.mean(distracted_test_rews_int) / np.mean(clean_test_rews_int)

        logger.info(f"Test set average KL {test_score}")
        summary = tf.Summary()
        summary.value.add(tag='Test_set_KL_score', simple_value=test_score)
    ...
    def update(self):
        ...
        # Calculate intrinsic returns and advantages. 分别计算内在激励产生的 return 和 advantage
        lastgaelam = 0
        ...
        # Calculate extrinsic returns and advantages. 分别计算外在激励产生的 return 和 advantage
        lastgaelam = 0
        for t in range(self.nsteps-1, -1, -1):      # nsteps-2 ... 0
            nextnew = self.I.buf_news[:, t + 1] if t + 1 < self.nsteps else self.I.buf_new_last
    ...
    def env_get(self, l):
        ...
        """
        # print("self.I.step_count:", self.I.step_count, ", self.I.env_results[l]:", self.I.env_results[l])
    ...
    @logger.profile("step")
    def step(self):           # 与环境交互, 并训练
        # 每次执行一次 step, self.I.step_count 加 1. self.nsteps=128. 当二者相等时，训练一次
    ...
    def save(self):
        ...
        exp_dir = os.path.join(save_dir, self.exp_name)
        saver = tf.train.Saver()
        save_path = saver.save(tf.get_default_session(), os.path.join(exp_dir,'agent.ckpt'))

        with open(os.path.join(exp_dir, 'eprews.pickle'), 'wb') as f:
            pickle.dump(self.ep_rews, f)

        print("Model saved in path: %s" % save_path)

    def load(self):
        saver = tf.train.Saver()
        saver.restore(tf.get_default_session(), os.path.join('checkpoints', self.model_to_load, "agent.ckpt"))
